...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        cerr << "command invalid! example: ./ChatClient 192.168.84.128 6000" << endl;
        exit(-1);
    }
    ...
    for (;;)
    {
        ...
        case 1: // login业务
        {
            ...
            // cin >> id;
            // cin.get(); // 读掉缓冲区残留的回车
            while (true) // 防止用户输入的id中有非数字字符
            {
                cout << "userid:";
                getline(cin, input); // 使用 getline 读取整行，包括空格

                // 尝试从字符串中读取整数
                std::istringstream iss(input);
                if (!(iss >> id) || !(iss.eof()))
                {
                    // 如果读取失败，或者读取后流中还有剩余字符（即存在非数字字符）
                    cerr << "输入包含非数字字符，用户id由数字组成，请重新输入 == Input contains non-digit characters,userid is all number, please re-enter" << endl;
                }
                else
                {
                    // 读取成功且没有剩余字符，退出循环
...
unordered_map<string, string> commandMap = {
    ...
    {"logoff", "退出登录，格式logoff"},
    {"deleteid", "注销账号，格式deleteid:id:password"}};
...
// "logoff" command handler
void logoff(int clientfd, string)
{
    json js;
    js["msgid"] = LOGOFF_MSG;
    js["id"] = g_currentUser.getId();
    ...
    if (-1 == len)
    {
        cerr << "send logoff msg error -> " << buffer << endl;
    }
    else
    {
